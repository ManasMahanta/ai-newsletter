...
export default function SavedList() {
  ...
  if (marks.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 bg-white/50 p-8 text-center dark:border-zinc-700 dark:bg-zinc-950/40">
        <p className="text-lg font-semibold">Nothing saved yet</p>
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          Hit <span className="font-medium">☆ Save</span> on any paper, tool, model,
          or job across the site and it&apos;ll appear here — stored privately in this
    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col divide-y divide-zinc-200 overflow-hidden rounded-xl border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">
        {marks.map((m) => (
          <li
            key={m.id}
            className="flex items-center gap-3 bg-white/65 px-5 py-3.5 dark:bg-zinc-950/55"
          >
            <a
              href={m.url}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 font-medium leading-snug hover:text-indigo-600 dark:hover:text-indigo-400"
            >
              {m.title}
            </a>
            <button
              ...
              onClick={() => remove(m.id)}
              aria-label="Remove"
              className="shrink-0 rounded-full border border-zinc-200 px-2 py-0.5 text-xs font-medium text-zinc-500 hover:border-rose-300 hover:text-rose-600 dark:border-zinc-700 dark:text-zinc-400"
            >
              Remove
            </button>
          </li>
