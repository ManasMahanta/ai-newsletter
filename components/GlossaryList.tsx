    ...
    <div className="flex flex-col gap-6">
        ...
        <nav aria-label="Jump to letter" className="flex flex-wrap gap-1.5">
          {letters.map((l) => (
            <a
              key={l}
              href={`#letter-${l}`}
              className="rounded-md px-2 py-0.5 font-mono text-sm font-semibold text-indigo-600 hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-500/10"
            >
              {l}
            </a>
          ))}
      ...
      {letters.map((letter) => (
        <section key={letter} id={`letter-${letter}`} className="scroll-mt-20">
          <h2 className="font-mono text-lg font-bold text-indigo-500 dark:text-indigo-400">
            {letter}
          </h2>
