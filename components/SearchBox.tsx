...
const KIND_STYLE: Record<SearchDoc["kind"], string> = {
  Issue: "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300",
  ...
  return (
    <div className="flex flex-col gap-5">
      <input
        ...
        className="w-full rounded-xl border border-zinc-200 bg-white/70 px-4 py-3 text-base outline-none placeholder:text-zinc-400 focus:border-indigo-400 dark:border-zinc-800 dark:bg-zinc-950/55 dark:focus:border-indigo-500"
      />
      ...
      <div className="flex flex-col divide-y divide-zinc-200 overflow-hidden rounded-xl border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-800">
        {results.map((doc, i) => (
          <Link
            key={`${doc.url}-${i}`}
            href={doc.url}
            className="group flex flex-col gap-1 bg-white/65 px-5 py-3.5 transition hover:bg-indigo-50/60 dark:bg-zinc-950/55 dark:hover:bg-indigo-500/10"
          >
            <div className="flex items-center gap-2">
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${KIND_STYLE[doc.kind]}`}>
                {doc.kind}
              </span>
              <span className="font-medium leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400">
                {doc.title}
              </span>
            </div>
