...
export default async function GlossaryTermPage({ params }: Props) {
  ...
  return (
    <article className="flex flex-col gap-6">
      ...
      {related.length > 0 && (
        <section className="rounded-xl border border-zinc-200 bg-white/65 p-5 dark:border-zinc-800 dark:bg-zinc-950/55">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {related.map((r) => (
              <Link
                ...
                className="rounded-full border border-zinc-200 px-3 py-1 text-sm text-zinc-600 transition hover:border-indigo-400 hover:text-indigo-600 dark:border-zinc-700 dark:text-zinc-400 dark:hover:border-indigo-500 dark:hover:text-indigo-400"
              >
                {r.term}
              </Link>
            ))}
      ...
      <InlineCTA heading="Understand AI, one term at a time" />
