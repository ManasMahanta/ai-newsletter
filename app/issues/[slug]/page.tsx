...
export default async function IssuePage({ params }: Props) {
  ...
  return (
    <article>
      ...
      <div className="prose prose-zinc mt-8 max-w-none dark:prose-invert prose-headings:tracking-tight prose-a:text-indigo-600 dark:prose-a:text-indigo-400">
        <MDXRemote source={issue.content} />
      </div>

      <InlineCTA heading="Get the next issue in your inbox" />

      <nav className="mt-10 flex flex-col gap-3 border-t border-zinc-200 pt-6 text-sm sm:flex-row sm:justify-between dark:border-zinc-800">
        {prev ? (
          <Link
        ...
        {next ? (
          <Link
            ...
            className="text-zinc-600 hover:text-indigo-600 sm:text-right dark:text-zinc-400 dark:hover:text-indigo-400"
          >
            Newer: {next.title} →
          </Link>
        ) : (
